...
export class JourneyService {
  ...
  async createEvent(
    ...
    const currentStatus = (driver as any).currentJourneyStatus;
    const newType = data.type;

    // Rule: Cannot start journey if already in one (unless it was ended)
    if (
      newType === 'JOURNEY_START' &&
      currentStatus &&
      currentStatus !== 'JOURNEY_END'
    ) {
    ...
    if (['MEAL_START', 'WAIT_START', 'REST_START'].includes(newType)) {
      ...
      if (['MEAL_START', 'WAIT_START', 'REST_START'].includes(currentStatus)) {
        throw new BadRequestException(
          'Você já está em uma pausa. Encerre-a primeiro.',
        );
      }
    }

    // Rule: Cannot end meal/rest/wait if not in that specific break
    if (newType === 'MEAL_END' && currentStatus !== 'MEAL_START')
      throw new BadRequestException(
        'Não há intervalo de refeição aberto para encerrar.',
      );
    if (newType === 'WAIT_END' && currentStatus !== 'WAIT_START')
      throw new BadRequestException(
        'Não há tempo de espera aberto para encerrar.',
      );
    if (newType === 'REST_END' && currentStatus !== 'REST_START')
      throw new BadRequestException('Não há descanso aberto para encerrar.');

    // Rule: Cannot end journey if in a break (must end break first)
    if (
      newType === 'JOURNEY_END' &&
      ['MEAL_START', 'WAIT_START', 'REST_START'].includes(currentStatus)
    ) {
      throw new BadRequestException(
        'Encerre a pausa atual antes de finalizar a jornada.',
      );
    }
    ...
    await (this.prisma as any).driver.update({
      ...
      data: {
        ...
        status: newType === 'JOURNEY_END' ? 'OFFLINE' : 'IDLE',
      },
    });
