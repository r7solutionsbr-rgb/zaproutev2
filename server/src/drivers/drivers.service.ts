...
export class DriversService {
  ...
  async importMassive(tenantId: string, drivers: any[]) {
    ...
    for (const d of drivers) {
      ...
      const existingDriver = await (this.prisma as any).driver.findFirst({
        where: { 
            cpf: d.cpf,
      ...
      if (existingDriver) {
        // ATUALIZA
        const updated = await (this.prisma as any).driver.update({
        ...
        // CRIA
        const created = await (this.prisma as any).driver.create({
          data: {
              name: d.name,
              cpf: d.cpf,
              email: d.email,
