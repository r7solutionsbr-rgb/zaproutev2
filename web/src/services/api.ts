...
export const api = {
  ...
  // --- ROTAS ---
  routes: {
    getAll: async (tenantId: string, days = 30) => {
      // <--- COM FILTRO DE DIAS
      const response = await client.get(
        `/routes?tenantId=${tenantId}&days=${days}`,
      );
      return response.data;
    },
    getDashboardStats: async (tenantId: string, days = 7) => {
      const response = await client.get(
        `/routes/dashboard-stats?tenantId=${tenantId}&days=${days}`,
      );
      return response.data;
    },
    getAllPaginated: async (
      ...
      const response = await client.get('/routes/paginated', { params });
      return response.data;
    },
  ...
  drivers: {
    ...
    getAllPaginated: async (
      ...
      const response = await client.get(`/drivers/paginated`, {
    ...
    import: async (tenantId: string, drivers: any[]) => {
      const response = await client.post('/drivers/import', {
        tenantId,
        drivers,
      });
      return response.data;
    ...
    delete: async (id: string) => {
      const response = await client.delete(`/drivers/${id}`);
      return response.data;
    },
  },
  // --- VEÍCULOS ---
  vehicles: {
    ...
    import: async (tenantId: string, vehicles: any[]) => {
      const response = await client.post('/vehicles/import', {
        tenantId,
        vehicles,
      });
      return response.data;
  ...
  customers: {
    ...
    import: async (tenantId: string, customers: any[]) => {
      const response = await client.post('/customers/import', {
        tenantId,
        customers,
      });
      return response.data;
  ...
  journey: {
    ...
    getHistory: async (driverId: string, date?: string) => {
      const response = await client.get(
        `/journey/${driverId}/history${date ? `?date=${date}` : ''}`,
      );
      return response.data;
    },
  },
  backoffice: {
    ...
    updateTenantStatus: async (id: string, status: string) => {
      const response = await client.patch(`/backoffice/tenants/${id}/status`, {
        status,
      });
      return response.data;
    },
  ...
  deliveries: {
    getAllPaginated: async (
      ...
      if (filters) {
        ...
        if (filters.status && filters.status !== 'ALL')
          params.status = filters.status;
        if (filters.priority) params.priority = filters.priority; // Novo filtro
        if (filters.driverId && filters.driverId !== 'ALL')
          params.driverId = filters.driverId;
        if (filters.search) params.search = filters.search;
      }
      const response = await client.get('/deliveries/paginated', { params });
      return response.data;
    },
  },
  // --- OCORRÊNCIAS (NOVO) ---
  occurrences: {
    getAllPaginated: async (
      ...
      const response = await client.get('/occurrences/paginated', { params });
      return response.data;
    },
  },
};
