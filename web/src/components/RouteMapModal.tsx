...
export const RouteMapModal: React.FC<RouteMapModalProps> = ({
  ...
  const focusPoint = focusDeliveryId
    ? points.find((p) => p.id === focusDeliveryId)
    : null;
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-200 bg-slate-50">
          <div>
            ...
            <p className="text-xs text-slate-500">
              {points.length} pontos de entrega mapeados
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              {points.map((p, idx) => (
                <Marker
                  ...
                  icon={
                    p.status === 'DELIVERED'
                      ? icons.green
                      : p.status === 'FAILED' || p.status === 'RETURNED'
                        ? icons.red
                        : icons.blue
                  }
                >
                  <Popup className="custom-popup">
                    <div className="p-1 min-w-[200px]">
                      ...
                      <div className="mb-2 border-b border-slate-100 pb-2">
                        <strong className="text-sm block text-slate-800">
                          {p.customerName}
                        </strong>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold ${p.status === 'DELIVERED'
