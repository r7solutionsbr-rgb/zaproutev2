...
export const AiChatWidget: React.FC = () => {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 transition-all duration-300 flex flex-col animate-in zoom-in-95 fade-in duration-200 ${isMinimized ? 'w-72 h-16' : 'w-80 md:w-[400px] h-[600px] max-h-[85vh]'}`}
    >
      {/* Header Premium */}
      <div
        className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 flex items-center justify-between text-white shrink-0 cursor-pointer shadow-md relative overflow-hidden"
        ...
        <>
          <div className="flex-1 overflow-y-auto p-4 bg-slate-50 space-y-6 custom-scrollbar">
            {messages.map((msg) => (
              ...
              >
                <div
                  className={`flex max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'} gap-2 items-end`}
                >
                  ...
                  <div
                    className={`p-3.5 text-sm shadow-sm relative group transition-all duration-200 hover:shadow-md ${
                      msg.sender === 'user'
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] block mt-1.5 text-right font-medium opacity-70 ${msg.sender === 'user' ? 'text-blue-100' : 'text-slate-400'}`}
                    >
          ...
          <div className="p-3 bg-white border-t border-slate-100">
            <form
              ...
              className="flex items-center gap-2 bg-slate-100 rounded-full px-2 py-2 border border-transparent focus-within:border-blue-300 focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-50 transition-all duration-200"
            >
              <input
                type="text"
                value={inputValue}
