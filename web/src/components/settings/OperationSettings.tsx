...
interface OperationSettingsProps {
  ...
  updateNestedConfig: (
    section: keyof TenantConfig,
    key: string,
    value: any,
  ) => void;
}

export const OperationSettings: React.FC<OperationSettingsProps> = ({
  config,
  updateConfig,
  updateNestedConfig,
}) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-bold text-slate-800 mb-6">
          Estratégia de Importação de Motoristas
        </h3>
          ...
          {[
            {
              value: 'CPF_ONLY',
              label: 'Apenas CPF',
              desc: 'Atualiza se o CPF existir. Mais permissivo.',
              icon: FileText,
            },
            {
              value: 'CPF_AND_NAME',
              label: 'CPF + Nome',
              desc: 'Exige match exato de CPF e Nome.',
              icon: Shield,
            },
            {
              value: 'STRICT',
              label: 'Estrita',
              desc: 'Rejeita duplicados. Mais seguro.',
              icon: Shield,
            },
          ].map((opt) => (
            <button
              ...
              className={`p-6 rounded-xl border-2 text-left transition-all ${
                config.driverImportStrategy === opt.value
            ...
            >
              <opt.icon
                size={24}
                className={`mb-3 ${config.driverImportStrategy === opt.value ? 'text-blue-600' : 'text-slate-400'}`}
              />
              <div
                className={`font-bold mb-1 ${config.driverImportStrategy === opt.value ? 'text-blue-700' : 'text-slate-700'}`}
              >
                {opt.label}
              </div>
          ...
          {[
            {
              value: 'SIMPLE',
              label: 'Simples',
              steps: ['Pendente', 'Entregue'],
            },
            {
              value: 'STANDARD',
              label: 'Padrão',
              steps: ['Início', 'Chegada', 'Entrega'],
            },
            {
              value: 'DETAILED',
              label: 'Detalhado',
              steps: ['Início', 'Chegada', 'Descarga', 'Fim'],
            },
          ].map((wf) => (
            <button
              key={wf.value}
              ...
              className={`p-4 rounded-xl border-2 text-left transition-all ${
                config.deliveryWorkflow === wf.value
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Motorista deve enviar foto para concluir.
                </div>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                checked={
                  config.displaySettings?.requireProofOfDelivery || false
                }
                onChange={(e) =>
                  updateNestedConfig(
                    'displaySettings',
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Aceitar devolução de itens no ato.
                </div>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Exibir valor das notas fiscais no app.
                </div>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Exibir volume das entregas no app.
                </div>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center justify-between mb-6">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) =>
                updateConfig('enableJourneyControl', e.target.checked)
              }
            />
            <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        ...
        <div
          className={`transition-opacity ${config.enableJourneyControl === false ? 'opacity-40 pointer-events-none' : ''}`}
        >
