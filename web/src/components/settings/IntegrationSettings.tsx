    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <button
            onClick={() =>
              updateNestedConfig('whatsappProvider', 'type', 'ZAPI')
            }
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">
                Conexão via QR Code. Simples e direto.
              </div>
            </div>
            {config.whatsappProvider?.type === 'ZAPI' && (
              <CheckCircle className="ml-auto text-green-600" />
            )}
          </button>

          <button
            onClick={() =>
              updateNestedConfig('whatsappProvider', 'type', 'SENDPULSE')
            }
            className={`p-6 rounded-xl border-2 text-left transition-all flex items-center gap-4 ${
              config.whatsappProvider?.type === 'SENDPULSE'
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">
                API Oficial. Requer aprovação de templates.
              </div>
            </div>
            {config.whatsappProvider?.type === 'SENDPULSE' && (
              <CheckCircle className="ml-auto text-blue-600" />
            )}
          </button>
        </div>
