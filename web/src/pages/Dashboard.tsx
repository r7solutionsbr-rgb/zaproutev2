...
export const Dashboard: React.FC = () => {
  ...
  // --- GREETING (MOVIDO PARA CIMA PARA NÃO QUEBRAR HOOKS) ---
  ...
  const getDriverName = (driverId?: string) => {
    if (selectedOccurrence?.driverName) return selectedOccurrence.driverName;
    // Fallback simples já que não temos lista completa de drivers aqui
    return 'Motorista';
  };
    ...
    return (
      <div className="p-6 space-y-6 max-w-7xl mx-auto animate-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[500px]">
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 flex flex-col space-y-4">
  ...
  // --- MOCK DATA FOR CHARTS (REMOVIDO, AGORA VEM DO HOOK) ---
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Aqui está o panorama operacional de hoje.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
          ...
          <span className="text-4xl font-bold text-slate-900">{active}</span>
        </div>
            ...
            {activeRouteProgress.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <EmptyState
                  ...
                  description="As rotas em andamento aparecerão aqui automaticamente."
                />
              </div>
            ) : (
              activeRouteProgress.map((route) => (
                <div
                  key={route.id}
                  className="group p-4 rounded-xl border border-slate-100 hover:border-blue-100 hover:bg-blue-50/30 transition-all"
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-right">
                      <span
                        className={`text-xl font-bold ${route.status === 'COMPLETED' ? 'text-green-600' : 'text-blue-600'}`}
                      >
                        {route.percentage}%
                      </span>
                    </div>
                  ...
                  <div className="flex justify-between mt-2 text-[10px] font-medium text-slate-400">
                    ...
                    <span>
                      {route.processed} / {route.total} entregas
                    </span>
                  </div>
                </div>
              ))
            ...
            {occurrences.length === 0 ? (
              <div className="h-full flex items-center justify-center px-4">
                <EmptyState
                  ...
                  description="Nenhum problema reportado até o momento."
                />
              </div>
            ) : (
              occurrences.map((item) => (
                <div
                  key={item.id}
                  className="p-3 bg-white border border-red-100 rounded-xl shadow-sm hover:shadow-md transition-all"
                >
                  <div className="flex justify-between items-start mb-1">
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${item.status === 'FAILED' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}
                    >
            ...
            <div className="p-6 bg-red-50 border-b border-red-100 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {/* NF removida pois não vem no payload simplificado, ou usar ID se tiver */}
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
              <div>
                ...
                <p className="font-bold text-slate-800 text-lg">
                  {selectedOccurrence.customerName}
                </p>
